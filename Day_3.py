...
def convert_instructions_into_path_set(list_of_instructions):
    x, y = 0, 0
    path = {(x, y)}
...
def convert_instructions_into_path_list(list_of_instructions):
    x, y = 0, 0
    path = [(x, y)]
    for instruction in list_of_instructions:
        if instruction[0] == 'R':
...
def calculate_path_length(path, point):
    path_length = 0
    for pt in path:
        if point == pt:
            return path_length
